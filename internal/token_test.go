...
func TestRetrieveToken_InParams(t *testing.T) {
	...
	defer ts.Close()
	_, err := RetrieveToken(context.Background(), clientID, "", ts.URL, url.Values{}, AuthStyleInParams, false, styleCache)
	if err != nil {
...
func TestRetrieveTokenWithContexts(t *testing.T) {
	...
	_, err := RetrieveToken(context.Background(), clientID, "", ts.URL, url.Values{}, AuthStyleUnknown, false, styleCache)
	if err != nil {
	...
	cancel()
	_, err = RetrieveToken(ctx, clientID, "", cancellingts.URL, url.Values{}, AuthStyleUnknown, false, styleCache)
	close(retrieved)
	if err == nil {
